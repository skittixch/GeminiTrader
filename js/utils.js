// js/utils.js
import state from "./state.js"; // Needs state for log scale check
...
/**
 * Formats a Unix timestamp (seconds) into HH:MM string (CST/CDT),
...
export function formatTimestamp(timestamp) {
  ...
  const options = {
    ...
    // hour: 'numeric', // Let hour12 handle format
    minute: "2-digit",
    hour12: state.is12HourFormat, // *** USE STATE FLAG ***
  };
  try {
    // Need hour specified for toLocaleString to include it
    options.hour = state.is12HourFormat ? "numeric" : "2-digit"; // Use 2-digit for 24h (00), numeric for 12h (12)
    let formatted = date.toLocaleString("en-US", options);
    // Optional: Remove the space before AM/PM if desired for compactness
    ...
    return `${h}:${m}`; // Fallback to simple 24h local
  }
}
